...
export default function AdminProductsPage() {
  ...
  return (
    <div>
      <div className="section-title">Quan ly san pham</div>

      {canWrite && (
        <div className="card" style={{ marginBottom: 20 }}>
          <h3 style={{ textAlign: 'left', marginTop: 0 }}>{editingId ? 'Sua san pham' : 'Them san pham moi'}</h3>
          <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div className="form-group">
              <label>Ten san pham (viet hoa chu dau)</label>
            ...
            <div style={{ gridColumn: '1 / -1', display: 'flex', gap: 8 }}>
              <button className="btn" type="submit">{editingId ? 'Luu thay doi' : 'Them san pham'}</button>
              {editingId && <button className="btn outline" type="button" onClick={resetForm}>Huy</button>}
            </div>
          </form>
        </div>
      )}
            ...
            {products.map((p) => (
              <tr key={p.productId}>
                ...
                <td style={{ display: 'flex', gap: 8 }}>
                  {canWrite && <a onClick={() => startEdit(p)}>Sua</a>}
                  {canDelete && <a onClick={() => handleDelete(p.productId)} style={{ color: 'var(--danger)' }}>Xoa</a>}
                </td>
              </tr>
            ))}
          </tbody>
